...
public class OrderDAO {
    ...
    public List<Order1> getAll() {
        ...
        String sql = "SELECT O.[id], A.[lastname], A.[phone], A.[address], O.[paymentID], P.[Name] AS PromotionName, OD.totalItems, OD.totalPrice * (100 - P.Value) / 100, OS.[orderDate], OS.[orderStatus]\n"
                + "             FROM [Order] O \n"
                + "             JOIN (\n"
    ...
    public List<OrderDetailUser> getOrderDetailUser(int customerID) throws SQLException, ClassNotFoundException {
        ...
        String sql = "SELECT P.name, OD.quantity, OD.price * (100 - P.discount) / 100 , OS.orderDate, OS.orderStatus "
                + "FROM [Order] O "
